...
class IndicadoresResumenController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $nombre = fn ($g, $doc) => $g->first(fn ($r) => ! empty($r->nombre))?->nombre
        ...
        $promedios = [
            ...
            // excesos_tiempo_ruta pasó a ser texto (hora del día): se toma la
            // proporción de jornadas con exceso como "promedio por jornada".
            'excesos' => $total > 0
                ? round($rows->filter(fn ($r) => filled($r->excesos_tiempo_ruta)
                    && ! in_array(trim((string) $r->excesos_tiempo_ruta), ['0', '00:00', '00:00:00', '0:00:00'], true))->count() / $total, 3)
                : null,
            'rmd' => $rows
        ...
        $porCargo = $rows->whereNotNull('cargo')
            ...
            ->map(fn ($g, $cargo) => [
                ...
                // excesos_tiempo_ruta ahora es texto (hora del día): se cuenta
                // cuántos registros traen un valor real en vez de sumarlo.
                'excesos_sum' => $g->filter(fn ($r) => filled($r->excesos_tiempo_ruta)
                    && ! in_array(trim((string) $r->excesos_tiempo_ruta), ['0', '00:00', '00:00:00', '0:00:00'], true))->count(),
